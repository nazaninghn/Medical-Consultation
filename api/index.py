...
try:
    ...
    # Load tools module
    tools_spec = importlib.util.spec_from_file_location("tools", os.path.join(os.path.dirname(os.path.dirname(__file__)), "tools.py"))
    tools_module = importlib.util.module_from_spec(tools_spec)
    tools_spec.loader.exec_module(tools_module)
    
    # Load schema module
    schema_spec = importlib.util.spec_from_file_location("schema", os.path.join(os.path.dirname(os.path.dirname(__file__)), "schema.py"))
    schema_module = importlib.util.module_from_spec(schema_spec)
    schema_spec.loader.exec_module(schema_module)
    
    tools = tools_module.tools
    convert_text_to_speech = tools_module.convert_text_to_speech
    process_uploaded_file = tools_module.process_uploaded_file
    convert_speech_to_text = tools_module.convert_speech_to_text
    SymptomResponse = schema_module.SymptomResponse
...
def chat():
    try:
        ...
        # Generate audio response (simplified for serverless)
        audio_path = None
        try:
...
def serve_audio(filename):
    try:
        return send_file(f'/tmp/audio/{filename}', as_attachment=False)
    except Exception as e:
